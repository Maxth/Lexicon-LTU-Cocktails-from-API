...
export function InfoPage() {
  ...
  const {getCocktailDetails} = useCocktailAPi();
  const [drinkDetails, setDrinkDetails] = useState<IDrinkDetails>();

  useEffect(() => {
    if (cocktailId) {
      getCocktailDetails(cocktailId).then(drinkDetails =>
  ...
  return (
    <section className='info-page'>
      <article className='container'>
        <div
          className='img'
          style={{backgroundImage: `url(${drinkDetails.imgUrl})`}}
        />
        <div className='info'>
          <h1 className='header'>{drinkDetails.name}</h1>
          <h4 className='sub-header'>Ingredients</h4>
          {drinkDetails.ingredients?.map((ingr, idx) => (
            <div className='ingr-container' key={`${ingr.ingr}-${idx}`}>
              <p className='ingredient'>{ingr.ingr}</p>
              <p className='measurement'>{ingr.msr || ''}</p>
            </div>
          ))}
          ...
          <div className='tag-container'>
            {drinkDetails.tags?.map((t, idx) => (
              <p key={`${t}-${idx}`} className='tag'>
                {t}
              </p>
            ))}
          </div>
